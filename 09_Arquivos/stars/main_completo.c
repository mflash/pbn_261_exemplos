...
void window2viewport(float x, float y, int* xv, int* yv, float xwmin, float xwmax, float ywmin, float ywmax, float xvmin, float xvmax, float yvmin, float yvmax);
...
int main(int argc, char *argv[])
{
    ...
    float xvmin = 0;
    float xvmax = out.width-1;
    float yvmin = 0;
    float yvmax = out.height-1;
    ...
    for(int i=0; i<totalPontos; i++) {
        ...
        window2viewport(stars[i].x, stars[i].y, &xv, &yv, xwmin, xwmax, ywmin, ywmax, xvmin, xvmax, yvmin, yvmax);
        draw_point(out.width, out.height, pout, xv, yv, white);
    }
